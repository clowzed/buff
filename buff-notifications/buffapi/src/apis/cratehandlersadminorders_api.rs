...
/// struct for typed errors of method [`cancel_order_by_id`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum CancelOrderByIdError {
...
/// struct for typed errors of method [`finish_order_by_id`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum FinishOrderByIdError {
    Status400(models::Details),
    Status401(models::Details),
    Status404(models::Details),
    Status500(models::Details),
...
pub async fn cancel_order_by_id(
    configuration: &configuration::Configuration,
    id: i64,
) -> Result<(), Error<CancelOrderByIdError>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<CancelOrderByIdError> =
            serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent {
            status: local_var_status,
            content: local_var_content,
...
pub async fn finish_order_by_id(
    configuration: &configuration::Configuration,
    id: i64,
) -> Result<(), Error<FinishOrderByIdError>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<FinishOrderByIdError> =
            serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent {
            status: local_var_status,
            content: local_var_content,
